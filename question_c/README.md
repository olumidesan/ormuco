...
- Install the libraries in the `requirements.txt` file (preferably in a virtual environment). (Mainly `sqlalchemy` and PostgreSQL's driver `psycopg2`). To do that, run `pip3 install -r requirements.txt`. On Ubuntu, it looks like this: `(venv) user@host:~/question_c$ pip3 install -r requirements.txt`.
...
- Directions for testing are in the README.md file in the test directory
